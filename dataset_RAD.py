...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    all_answers = cPickle.load(open(answer_path, 'rb'))
    all_answers = sorted(all_answers, key=lambda x: x['qid'])

    answers = []
    q_id_list = [i['qid'] for i in samples]
    # print(len(q_id_list))
    for i in range(len(all_answers)):
        if all_answers[i]['qid'] in q_id_list:
    ...
    #         if end > length:
    #             break
    #         new_entries=new_entries+A[begin:end]
    #         # print(len(new_entries))
    #         new_entries=new_entries+(B[begin:end])
    #         # print(len(new_entries))
    #         new_entries=new_entries+(C[begin:end])
    #         # print(len(new_entries))
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        self.args = args
        assert name in ['train', 'test', 'new', 'new_train', 'balanced_new_train']
    ...
    def __getitem__(self, index):
        ...
        if None!=answer:
            ...
            return image_data, question, target, answer_type, question_type, phrase_type, entry['qid'], entry['image_name'], entry['organ_type']

        else:
            return image_data, question, answer_type, question_type, phrase_type, entry['qid'], entry['image_name'], entry['organ_type']
