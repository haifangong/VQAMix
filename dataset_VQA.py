...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        if args.maml:
            # TODO: load images
            images_path = os.path.join(dataroot, 'pytorch_images84.pkl')
        ...
        if args.autoencoder and args.maml:
            self.v_dim = args.feat_dim + args.feat_dim
    ...
    def __getitem__(self, index):
        ...
        if self.args.maml:
            if 'RAD' in self.args.RAD_dir:
                ...
                maml_images_data = self.maml_images_data[entry['image']].reshape(3 * 84 * 84)
        ...
        if None!=answer:
            ...
            return image_data, question, target, answer_type, question_type, entry['phrase_type'], entry['qid'], entry['image_name'], entry['organ_type']
